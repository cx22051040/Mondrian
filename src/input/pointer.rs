use smithay::{
    ...
    utils::{
        Logical, Point, Serial, SERIAL_COUNTER
    }, 
    wayland::{
...
impl GlobalData {
    pub fn on_pointer_motion<I: InputBackend>(&mut self, event: I::PointerMotionEvent) {
        ...
        let under = self.surface_under(position, serial, true);
        ...
        if pointer_locked {
            ...
            let new_under = self.surface_under(clamp_position, serial, true);
    ...
    pub fn on_pointer_button<I: InputBackend>(&mut self, event: I::PointerButtonEvent) {
        ...
        let position = pointer.current_location();
        if let Some((wl_surface, loc)) = self.surface_under(position, serial, need_focus) {
            // grab and resize
    ...
    fn surface_under(&mut self, position: Point<f64, Logical>, serial: Serial, need_focus: bool) -> Option<(WlSurface, Point<f64, Logical>)> {
        // TODO: remove clone
            ...
            .or_else(|| layer_map.layer_under(WlrLayer::Top, position - output_geo.loc.to_f64()))
        {
            ...
            if let Some((surface, surface_loc)) = layer.surface_under(
                ...
                if layer.can_receive_keyboard_focus() && need_focus {
                    self.set_keyboard_focus(Some(surface.clone()), serial);
                }

                return Some((
                    surface,
                    surface_loc.to_f64() + layer_loc
                ));
            }
        } 
        
        // The window
        else if let Some((surface, surface_loc)) = self
            .workspace_manager
            .surface_under(position, need_focus) {
                if need_focus {
                    self.set_keyboard_focus(Some(surface.clone()), serial);
                }
                return Some((surface, surface_loc))
            }
