...
_Mondrain_ 的核心目标是实现一个 #standout[面向未来的、结构可控的] 平铺式桌面环境，因此我们选择了 Rust 作为主要开发语言，并基于 Smithay 框架进行构建。这一组合在性能、可靠性、安全性和协议支持方面表现出极高的适配性。
...
在 _Mondrain_ 中，我们实现了一个轻量且可扩展的全局快捷键匹配系统，用于：
...
为了实现桌面组件如状态栏、启动器、壁纸容器等持久性窗口，_Mondrain_ 支持 _*wlr-layer-shell*_ 协议。这一协议最初由 wlroots 提出，是实现 Wayland 桌面环境中“系统层级窗口”的标准手段。
